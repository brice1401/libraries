...
bool BH1745NUC::begin()
{
	...
	Wire.beginTransmission(bh_i2cAddress);
	error = Wire.endTransmission();
	
	
	if(error == 0){
		// the communication is succesful
		return(true);
	}
	return(false); // the communication has failed
	/*
...
BH1745NUC::BH1745NUC(){
  //constructor
    ...
    getAddr_BH1745NUC(BH1745NUC_DEFAULT_ADDRESS);    // 0x38, 0111000
    // getAddr_BH1745NUC(BH1745NUC_ADDRESS_HIGH);    // 0x39, 0111001
    ...
    setSWReset(SW_RESET_NOT_START);                  // Initial reset is not started
    // setSWReset(SW_RESET_START);                   // Initial reset is started

    setMeasTime(MEAS_TIME_160);                      // 000 : 160 msec
    // setMeasTime(MEAS_TIME_320);                   // 001 : 320 msec
    // setMeasTime(MEAS_TIME_640);                   // 010 : 640 msec
    // setMeasTime(MEAS_TIME_1280);                  // 011 : 1280 msec
    // setMeasTime(MEAS_TIME_2560);                  // 100 : 2560 msec
    // setMeasTime(MEAS_TIME_5120);                  // 101 : 5120 msec

    setRGBCValid(RGBC_VALID_UPDATE);                 // RGBC data is updated after last writing MODE_CONTROL1,2 register or last reading MODE_CONTROL2 register
    // setRGBCValid(RGBC_VALID_NOT_UPDATE);          // RGBC data is not updated after last writing MODE_CONTROL1,2 register or last reading MODE_CONTROL2 register

    setRGBCEnable(RGBC_EN_ACTIVE);                   // RGBC measurement is active
    // setRGBCEnable(RGBC_EN_INACTIVE);              // RGBC measurement is inactive and becomes power down

    setADCGain(ADC_GAIN_1X);                         // 00 : 1X
    // setADCGain(ADC_GAIN_2X);                      // 00 : 2X
    // setADCGain(ADC_GAIN_16X);                     // 00 : 16X

    setINTStatus(STATUS_INACTIVE);                   // Interrupt signal is inactive
    // setINTStatus(STATUS_ACTIVE);                  // Interrupt signal is active

    setINTLatch(INT_LATCH_LATCHED);                  // INTERRUPT pin is latched until INTERRUPT register is read or initialized
    // setINTLatch(INT_LATCH_UPDATED);               // INTERRUPT pin is updated after each measurement

    setINTEnable(INT_ENABLE_DISABLE);                // INTERRUPT pin disable
    // setINTEnable(INT_ENABLE_ENABLE);              // INTERRUPT pin ensable

    setPersistance(PERSISTENCE_UPDATED_EACH);        // Interrupt status is updated at each measurement end
    // setPersistance(PERSISTENCE_UPDATED_EACH);     // Interrupt status is toggled at each measurement end
    // setPersistance(PERSISTENCE_UPDATED_EACH);     // Interrupt status is updated if 4 consecutive threshold judgments are the same
    // setPersistance(PERSISTENCE_UPDATED_EACH);     // Interrupt status is updated if 8 consecutive threshold judgments are the same
}
